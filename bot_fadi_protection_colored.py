...
# إرسال خام حتى ندعم style: primary / danger للأزرار الملونة
def raw_api(method, payload):
    try:
...
def raw_send_message(chat_id, text, reply_markup=None, reply_to_message_id=None):
    ...
    return raw_api("sendMessage", payload)
...
def raw_send_photo(chat_id, photo, caption, reply_markup=None):
    ...
    if reply_markup:
        payload["reply_markup"] = reply_markup
    return raw_api("sendPhoto", payload)

def raw_edit_message(chat_id, message_id, text, reply_markup=None):
    payload = {"chat_id": chat_id, "message_id": message_id, "text": text, "parse_mode": "HTML"}
    if reply_markup:
        payload["reply_markup"] = reply_markup
    return raw_api("editMessageText", payload)

def blue_btn(text, url=None, callback_data=None):
    btn = {"text": text, "style": "primary"}
    if url:
        btn["url"] = url
    if callback_data:
...
def red_btn(text, url=None, callback_data=None):
    btn = {"text": text, "style": "danger"}
    if url:
        btn["url"] = url
    if callback_data:
        btn["callback_data"] = callback_data
...
def main_menu():
    return {
        "inline_keyboard": [
            [blue_btn("الحماية", callback_data="help_locks"), blue_btn("الأدمنية", callback_data="help_admin")],
            [blue_btn("المدراء", callback_data="help_manager"), blue_btn("المنشئ", callback_data="help_creator")],
            [blue_btn("المالك", callback_data="help_owner"), blue_btn("الأعضاء", callback_data="help_members")],
            [blue_btn("البنك", callback_data="help_bank"), blue_btn("الألعاب", callback_data="help_games")],
            [blue_btn("التسلية", callback_data="help_entertainment"), blue_btn("التحشيش", callback_data="help_fun")],
            [blue_btn("المطور", callback_data="help_dev"), blue_btn("الميوزك", callback_data="help_music")]
        ]
    }
...
def start_buttons():
    return {
        "inline_keyboard": [
            ...
            [blue_btn("الأوامر", callback_data="commands")]
        ]
    }
...
def send_commands(message, section=None):
    ...
    return raw_send_message(message.chat.id, "📚 اختر قسم الأوامر:", main_menu(), message.message_id)
...
def callbacks(call):
    ...
    if call.data in sections:
        return raw_edit_message(call.message.chat.id, call.message.message_id, COMMAND_TEXTS[sections[call.data]], main_menu())
...
def handler(message):
    ...
    if text.startswith("يوت "):
        ...
        try:
            ...
            sent_audio = bot.send_audio(message.chat.id, audio_url, title=title, performer="Aurelius", caption=f"🎧 {title}", reply_to_message_id=message.message_id)
            raw_send_message(
                message.chat.id,
                "SOURCE FADI",
                {"inline_keyboard": [[red_btn("SOURCE FADI", url=f"https://t.me/{DEV_USERNAME}")]]}
            )
            return sent_audio
        except Exception as e:
            print("MUSIC ERROR:", e)
