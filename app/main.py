...
from app.help import help_menu, get_help_text
from app.music import send_music
from app.weather import get_weather
from app.ai import ask_ai
from app.whisper import create_whisper, handle_whisper
from app.id import get_id_text
...
def get_members(chat_id):
    return list(members_cache.get(str(chat_id), []))
...
def handler(message: Message):
    ...
    if text.startswith("يوت "):
        return send_music(bot, message, text.replace("يوت ", "", 1))

    if text.startswith("طقس "):
        return bot.reply_to(message, get_weather(text.replace("طقس ", "", 1)))

    if text.startswith("زنجي "):
        return bot.reply_to(message, ask_ai(text.replace("زنجي ", "", 1)))

    if text.startswith("همسة ") or text.startswith("همسه ") or text.startswith("هم "):
        return create_whisper(bot, message)

    if text in GAME_NAMES:
        q, kb = make_game(text)
        return bot.send_message(message.chat.id, q, reply_markup=kb)

    if text == "الاسرع":
        word = start_fast(message.chat.id)
        return bot.send_message(message.chat.id, f"اول واحد يكتب:\n{word}")
    ...
    if text in ["زوجني", "ز"]:
        partner = marry_user(message.chat.id, message.from_user.id, get_members(message.chat.id))
        if not partner:
            return bot.reply_to(message, "ماكو اعضاء كفاية")
        return bot.reply_to(message, f"تم زواجك من:\n{partner}")

    if text in ["نداء", "نن"]:
        user = call_user(message.chat.id, get_members(message.chat.id))
        if not user:
            return bot.reply_to(message, "ماكو اعضاء")
        return bot.reply_to(message, f"نداء للعضو:\n{user}")
